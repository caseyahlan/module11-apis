...
seriesyears <- GET("http://data.unhcr.org/api/stats/time_series_years.json")
exdata <- content(apidata,"text")
years <- fromJSON(exdata)
years

# What is the "country code" for the "Syrian Arab Republic"?
countries <- GET("http://data.unhcr.org/api/countries/list.json")
countrycodes <- fromJSON(content(countries, "text"))
filter(countrycodes, name_en == "Syrian Arab Republic") %>% select(country_code)
